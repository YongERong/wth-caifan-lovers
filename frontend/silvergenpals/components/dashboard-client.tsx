...
export default function DashboardClient({ user }: DashboardClientProps) {
  ...
  const handleProfileCreated = async () => {
    ...
    checkProfile(); // Refresh the profile data
  };
  ...
  if (!profile) {
    return <ProfileCreation user={user} onProfileCreated={handleProfileCreated} />;
  }
      ...
      <div className="space-y-8 pt-6">
        ...
        <div className="bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-2xl p-8">
          <h1 className="text-3xl font-bold mb-2">
            Welcome back, {profile.first_name || user.user_metadata?.display_name || user.email?.split('@')[0]}! 👋
          </h1>
